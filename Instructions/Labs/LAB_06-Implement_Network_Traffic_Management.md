...
1. 添加一个对等互连，设置如下（其他设置保留默认值），然后单击“添加”：
    ...
    | 允许“az104-06-vnet01”访问对等互连的虚拟网络 | **确保选中该框（默认）** |
    | 允许“az104-06-vnet01”中的网关将流量转发到对等互连的虚拟网络 | **确保选中该框** |
    | 远程虚拟网络: 对等互连链接名称 | az104-06-vnet2_to_az104-06-vnet01 |
    | 虚拟网络部署模型 | **资源管理器** |
    | 我知道我的资源 ID | enabled |
    | 资源 ID | 在本任务前面部分记下的 az104-06-vnet2 的 resourceID 参数值。 |
    | 允许 az104-06-vnet2 访问 az104-06-vnet01 | **确保选中该框（默认）** |
    | 允许 az104-06-vnet2 接收转发自 az104-06-vnet01 的流量 | **确保选中该框** |
...
1. 添加一个对等互连，设置如下（其他设置保留默认值），然后单击“添加”：
    ...
    | 虚拟网络部署模型 | **资源管理器** |
    | 我知道我的资源 ID | enabled |
    | 资源 ID | 之前在本任务中记录的 **az104-06-vnet3** 的 resourceID 参数值。 |
    | 允许 az104-06-vnet3 访问 az104-06-vnet01 | **确保选中该框（默认）** |
    | 允许 az104-06-vnet3 接收转发自 az104-06-vnet01 的流量 | **确保选中该框** |
...
1. 单击“运行诊断测试”，然后等待至返回连接性检查结果。 验证“状态”是否为“成功”。 查看网络路径，请注意，连接是直接的，VM 之间没有中间跃点。
...
1. 单击“运行诊断测试”，然后等待至返回连接性检查结果。 请注意，状态为“失败”。
...
1. 在“网络观察程序”-“连接故障排除”边栏选项卡中采用以下设置（将其他设置保留为默认值）：
...
1. 单击“运行诊断测试”，然后等待至返回连接性检查结果。 验证“状态”是否为“成功”。 查看网络路径，并注意流量通过 10.60.0.4路由，而该 IP 地址已分配给 az104-06-nic0 网络适配器。 如果状态为“失败”，则应停止，然后重启 az104-06-vm0。
...
1. 在“添加公共 IP 地址”弹出窗口中，采用以下设置并单击“确定”，然后单击“添加”  。 完成后，单击“下一步: 后端池”。 
