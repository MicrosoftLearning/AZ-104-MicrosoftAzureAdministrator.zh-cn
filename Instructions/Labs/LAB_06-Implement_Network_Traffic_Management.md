...
在此任务中，你将使用模板来部署一个虚拟网络、一个网络安全组和两个虚拟机。
...
1. 选择“+ 添加负载均衡规则”。 添加负载均衡规则并进行以下设置（其他设置保留默认值）。  配置规则时，使用信息图标了解每个设置。 完成后，单击“保存”****。
...
1. 在“虚拟网络”边栏选项卡上，在虚拟网络列表中单击“az104-vnet1”********。

1. 在“az104-vnet1”虚拟网络边栏选项卡的“设置”部分，单击“子网”，然后单击“+ 子网”****************。
...
1. 在“基本信息”选项卡上，指定以下设置（其他设置保留默认值）：
    ...
    ![创建应用网关页的屏幕截图。](../media/az104-lab06-create-appgw.png)
...
1. 单击“下一步: 后端 >”，然后单击“添加后端池”。 指定以下设置（其他设置保留默认值）。 完成后，单击“添加”。
    ...
    | 名称 | `az104-appgwbe` |
    | 添加没有目标的后端池 | **否** |
    | 虚拟机 | **az104-rg6-nic1 (10.60.1.4)** |
    | 虚拟机 | **az104-rg6-nic2 (10.60.2.4)** |
...
1. 在“基于路径的路由”部分中，选择“添加多个目标以创建基于路径的规则”********。 将创建两个规则。 单击第一个规则后的“添加”，然后添加第二个规则****。 
...
1. 请务必保存并检查更改，然后选择“下一步”******：** 标记 >”。 无需任何更改。
