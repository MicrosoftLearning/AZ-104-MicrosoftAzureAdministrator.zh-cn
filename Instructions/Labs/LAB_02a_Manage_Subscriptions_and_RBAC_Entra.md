...
本实验室需要具有创建用户、创建自定义 Azure 基于角色的访问控制 (RBAC) 角色，以及将这些角色分配给用户的权限。 并非所有实验室主机托管服务提供商都提供此功能。 请询问讲师是否可以使用本实验室。
...
- 向指定用户授予为管理组中的所有订阅提交支持请求的权限。 该用户的权限应仅限于： 
...
1. 查看“管理组”边栏选项卡顶部的信息。 如果看到以下信息：“你已注册为目录管理员，但没有访问根管理组的必要权限”****，请执行下面的一系列步骤：

    1. 在 Azure 门户中，搜索并选择“Microsoft Entra ID”。
    
    1.  在显示租户属性的边栏选项卡上，在左侧垂直菜单的“管理”部分中，选择“属性” 。
    
    1.  在租户的“属性”边栏选项卡上的“Azure 资源的访问管理”部分中，选择“是”，然后选择“保存”   。
...
在此任务中，你将创建一个用户，将在上一个任务中创建的 RBAC 角色分配给该用户，并验证该用户可以执行 RBAC 角色定义中指定的任务。

1. 在 Azure 门户中，搜索并选择“Microsoft Entra ID”，单击“用户”，然后单击“+ 新建用户”  。
...
1. 在 Azure 门户中，搜索并选择“Microsoft Entra ID”，然后单击“用户” 。
...
1. 在 Cloud Shell 窗格中，运行以下命令以删除自定义角色定义的分配（将 `[object_ID]` 占位符替换为你在此任务的前面部分复制的“az104-02-aaduser1”用户帐户的“对象 ID”属性的值） ：
...
1. 在 Azure 门户中，返回“Microsoft Entra ID”的“用户 - 所有用户”边栏选项卡，然后删除“az104-02-aaduser1”用户帐户。
