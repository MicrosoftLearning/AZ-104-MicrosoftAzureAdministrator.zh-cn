...
1. 将虚拟机大小设置为“DS1_v2”，然后单击“重设大小”********。 出现提示时，请确认更改。

    >**注意**：如果“DS1_v2 标准”不可用，请选择其他大小。 重设大小也称为垂直纵向扩展或纵向缩减。
...
1. 在“网络”页上，单击“虚拟网络”文本框下方的“创建虚拟网络”链接，使用以下设置创建新的虚拟网络（其余设置保留默认值）************。  完成后，选择“确定”。

    | 设置 | 值 |
    | --- | --- |
    | 名称 | `vmss-vnet` |
    | 地址范围 | `10.82.0.0/20`（更改此处的值） |
    | 子网名称 | `subnet0` |
    | 子网范围 | `10.82.0.0/24` |
...
1. 在“缩放”**** 选项卡上，指定以下设置（其他设置保留默认值），然后单击“下一步:**** 管理 >”：
...
1. 让我们创建一条自动增加 VM 实例数的规则。 当平均 CPU 负载在 10 分钟内大于 70% 时，此规则会横向扩展。 触发规则时，VM 实例数增加 20%。
