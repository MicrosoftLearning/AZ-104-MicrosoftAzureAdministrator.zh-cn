...
1. 在“虚拟机”页中，选择“创建”，然后选择“虚拟机”********。

1. 在“基本信息”选项卡上，使用以下信息填写表单，然后选择“下一步:**** 磁盘 >”。 对于未指定的任何设置，保留默认值。
    ...
    | 可用性选项 | 没有所需的基础结构冗余 |
    | 安全类型 | **标准** |
    | 图像（查看所有图像） | Windows Server 2019 Datacenter - x64 Gen2****（请注意其他选项） |
    | 大小 | Standard_DS2_v3**** |
    | 用户名 | `localadmin` | 
    | 密码 | 提供复杂的密码**** |
    | 公共入站端口 | **无** |
...
1. 在“CoreServicesVnet”的“对等互连”中，选择“**+ 添加**”。 如果未指定，请使用默认值。 
    ...
    | 虚拟网络    | ManufacturingVnet (az104-rg5)****  |
    | 允许 'CoreServicesVnet' 访问 'ManufacturingVnet'            | 已选择（默认） |
    | 允许 'CoreServicesVnet' 接收来自 'ManufacturingVnet' 的转发流量 | 已选定 |
...
在此任务中，需要控制外围子网与内部核心服务子网之间的流量。 虚拟网络设备将安装在外围子网中，所有流量都应路由到该位置。 
...
1. 依次选择“**子网**”、“**+ 关联**”。 完成配置。
    ...
    | 设置 | 值 | 
    | --- | --- |
    | 虚拟网络 | CoreServicesVnet (az104-rg5)**** |
    | 子网 | **核心** |    
