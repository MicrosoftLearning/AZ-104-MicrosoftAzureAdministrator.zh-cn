...
1. 在分配列表中，单击表示“要求存在值为 Default 的 Cost Center 标记”策略分配的行中的省略号图标，然后使用“删除分配”菜单项删除分配********。
...
+ Azure 标记是由键-值对组成的元数据。 标记描述环境中的特定资源。 尤其需要指出的是，Azure 中的标记功能使你能够以逻辑方式标记资源。
