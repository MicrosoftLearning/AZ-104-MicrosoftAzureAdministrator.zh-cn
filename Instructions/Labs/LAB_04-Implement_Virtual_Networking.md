...
**CoreServicesVnet** 虚拟网络具有的资源量最多。 预计会有大量增长，因此该虚拟网络需要较大的地址空间。
...
1. 移动到“IP 地址”选项卡****。
    ...
    | IPv4 地址空间| `10.20.0.0/16`（分隔条目）|

1. 选择“+添加子网”。 完成每个子网的名称和地址信息。 请务必为每个新子网选择“添加”****。 
...
1. 在门户中，搜索并选择“部署自定义模板”。****

1. 选择“在编辑器中生成自己的模板”，然后选择“加载文件”。********

1. 选择包含制造业更改的 **templates.json** 文件，然后选择“保存”****。
...
1. 选择“+记录集”。 为每个需要专用名称解析支持的虚拟网络添加虚拟网络链接记录。
...
1. 选择“确定”并验证 **contoso.com** 是否具有名为 **www** 的 A 记录集。****
...
1. 从“概述”边栏选项卡中选择“+ 记录集”。******** 现在，将为需要专用名称解析支持的每台虚拟机添加一条记录。
...
如果使用的是 **自己的订阅**，需要一分钟时间才能删除实验室资源。 这将确保资源得到释放，并在最大程度上降低成本。 删除这些资源最简单的方法是删除实验室资源组。 

+ 在 Azure 门户中，选择资源组，然后选择“删除资源组”，**输入资源组名称**，然后单击“删除”。********
+ 使用 Azure PowerShell `Remove-AzResourceGroup -Name resourceGroupName`。
+ 使用 CLI `az group delete --name resourceGroupName`。
 
## 关键结论

恭喜，你已完成本实验室的内容。 下面是本模块的主要收获。 
