...
+ 任务 2：编辑一个 Azure 资源管理器模板并重新部署该模板。
+ 任务 3：配置 Cloud Shell 并使用 Azure PowerShell 部署模板。
+ 任务 4：使用 CLI 部署模板。 
...
1. 在编辑器窗格中，进行这些更改。
    ...
    + 将 **az104_disk1** 更改为 `az102_disk2`（需要更改一处）
...
1. 在“你没有装载任何存储”屏幕中，选择“显示高级设置”，然后提供必填信息。******** 
    ...
    |  -- | -- |
    | 资源组 | **az104-rg3** |
    | 存储帐户（新建） | `sacloudshell`（必须全局唯一，长度为 3 到 24 个字符，只能使用数字和小写字母） |
...
1. 验证你的文件是否在 Cloud Shell 存储中可用。 
    ...
    dir
    ```
    >**注意**：如果需要，可以使用 **cls** 来清除命令窗口。 可以使用箭头键移动命令历史记录。
   
1. 选择“编辑器”（大括号）图标并导航到 parameters JSON 文件。****
